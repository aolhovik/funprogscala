...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase.toList.groupBy((c: Char) => c) map { case (c, lst) => (c, lst.length) }).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((c, freq), acc) => {
        acc ++ (for {comb <- acc; n <- 1 to freq} yield (c, n) :: comb)
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char, Int]() ++ x) { case (map, (c, freq)) => {
      val newFreq = map(c) - freq
      if (newFreq != 0) map updated(c, newFreq)
      else map - c
    }
    }.toList
  }
